...
climat$solreducrech <- fct_recode(factor(climat$solreducrech), "Set an example (reduce emissions by more than one-third)"= "La recherche doit montrer l'exemple", "Reduce emissions by around one-third"="La recherche doit réduire ses émissions d'un tiers", "Benefit from an exemption (reduce emissions by less than one-third)"= "La recherche publique peut bénéficier d'un statut dérogatoire")
...
chisq.test(table(climat$preoccupe2 == "Extremely concerned", climat$NumVague)[, c(1, 5)])
```
...
chisq.test(table(climat$opinionecolo.cata == "Completely agree", climat$NumVague)[, c(1, 5)])
```
...
chisq.test(table(climat$opinionecolo.contraintes == "Completely agree", climat$NumVague)[, c(1, 5)])
```
...
chisq.test(table(climat$opinionecolo.proteger == "Completely agree", climat$NumVague)[, c(1, 5)])
```
...
chisq.test(table(climat$solreducrech3 == "More than one-third", climat$NumVague)[, c(1, 5)])
```
...
chisq.test(table(climat$solinstit.limitevols == "It is a priority", climat$NumVague)[, c(1, 5)])
```
...
chisq.test(table(climat$solinstit.selection == "It is a priority", climat$NumVague)[, c(1, 5)])
```
...
chisq.test(table(climat$solinstit.conf == "It is a priority", climat$NumVague)[, c(1, 5)])
```

- Finance train tickets even if more expensive or requires longer stay
```{r echo=FALSE}
knitr::kable(round(cprop(table(climat$solinstit.train, climat$NumVague))))
chisq.test(table(climat$solinstit.train == "It is a priority", climat$NumVague)[, c(1, 5)])
```
...
ANOVA and post-hoc Tukey-Kramer test:
```{r echo=FALSE}
lm(volsdist_tot ~ discipline_agr4, climat_recherche, contrasts=list(discipline_agr4="contr.Sum")) %>%
  tbl_regression(label=list(discipline_agr4 ~ "Discipline",
                            "(Intercept)" ~ "Intercept (Grand mean)"),
                 intercept=TRUE) %>% 
  bold_labels() %>%
...
chisq.test(table(climat$sitpro_reduite, climat$ordis.5anstotalproplus0)[c("Senior researcher", "Adjunct lecturer"),])
```
...
dat <- rbind(data.frame(variable="Reduce some of the advantages offered to you\nby your profession", valeur=climat$solrisqreducavion.avantages),
    ...
    mutate(variable=factor(variable, levels=unique(variable)),
           valeur=fct_relevel(valeur, c("It is unlikely", "It is likely but it is not a problem", "No opinion", "It is likely and it is a problem")))
...
```{r}
chisq.test(table(climat$discipline_agr4, climat$solrisqreducavion.qual, exclude=c(NA, "Not concerned")))

chisq.test(table(climat$discipline_agr4, climat$solrisqreducavion.donnees, exclude=c(NA, "Not concerned")))
chisq.test(table(climat$discipline_agr4, climat$solrisqreducavion.donnees, exclude=c(NA, "Not concerned")), simulate.p.value=TRUE, B=10000)

chisq.test(table(climat$sitpro_reduite, climat$solrisqreducavion.diffusion, exclude=c(NA, "Not concerned")))
chisq.test(table(climat$sitpro_reduite, climat$solrisqreducavion.diffusion, exclude=c(NA, "Not concerned")), simulate.p.value=TRUE, B=10000)

chisq.test(table(climat$sitpro_reduite, climat$solrisqreducavion.isoler, exclude=c(NA, "Not concerned")))
chisq.test(table(climat$sitpro_reduite, climat$solrisqreducavion.isoler, exclude=c(NA, "Not concerned")), simulate.p.value=TRUE, B=10000)
```
