...
```{r echo=FALSE}
#aumoinsunecochee <- rowSums(!is.na(select(climat, starts_with("covoitpqpas"))))
#Pour avoir le nombre de gens ayant coché au moins une case : aumoinsunecochee <- colSums(!is.na(select(climat, starts_with("covoitpqpas"))))
Covoit<- climat %>% 
...
#A vérifier : 1216 personnes ont au moins coché une case à cette question
   mutate(prop=n/1216)
