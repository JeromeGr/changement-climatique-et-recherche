...
climat$NumVague <- fct_recode(factor(climat$NumVague), "After initial message and before first reminder"= "Après premier message", "Between first and second reminders"="Après première relance", "Between second and third reminders"= "Après deuxième relance", "Between third and fourth reminders"= "Après troisième relance", "After fourth reminder"= "Après quatrième relance")
...
climat$acthum2 <- fct_recode(factor(climat$acthum2), "Yes, they are the only cause"="Oui, elles en sont l'unique cause","Yes, they play a large role"="Oui, elles jouent un grand rôle","Yes, they play a small role"="Oui, elles jouent un petit rôle", "No, they play no role"="Non, elles ne jouent aucun rôle", "There is no climate change"="Il n'y a pas de changement climatique","No opinion"="Sans opinion")
...
climat$pluspreoccupe <- fct_recode(factor(climat$pluspreoccupe), "Much more"="Beaucoup plus","A little more"="Un peu plus","Neither more nor less"="Ni plus ni moins", "A little less"="Un peu moins", "Much less"="Beaucoup moins","No opinion"="Sans opinion")
...
climat$opinionecolo.effondrement <- fct_recode(factor(climat$opinionecolo.effondrement), "Completely agree"="Tout à fait d'accord", "Somewhat agree"="Plutôt d'accord", "Somewhat disagree"="Plutôt pas d'accord", "Completely disagree"="Pas du tout d'accord", "No opinion"="Sans opinion")
...
climat$chgtpratique <- fct_recode(factor(climat$chgtpratique), "Yes, completely agree"="Oui, tout à fait d'accord", "Yes, somewhat agree"="Oui, plutôt d'accord", "No, somewhat disagree"="Non, plutôt pas d'accord", "No, completely disagree"="Non, pas du tout d'accord", "No opinion"="Sans opinion")
...
## Human activities and climage change
```{r echo=FALSE}
climat$acthum3 <- fct_collapse(climat$acthum2,
                               "Yes, they play a large role\n or are the only cause"=c("Yes, they are the only cause",                                  "Yes, they play a large role"))
...
ggplot(drop_na(climat, acthum3),
  ...
  source_champ(nrow(drop_na(climat, acthum3)),
               "Interpretation: For 95.1% of respondents, human activities play a major role or is the only cause of climate change.")
```

```{r echo=FALSE}
climat$acthum3 <- fct_collapse(climat$acthum2,
                               "Yes, they play a large role or are the only cause"=c("Yes, they are the only cause",                                  "Yes, they play a large role"))
...
ggplot(drop_na(climat, pluspreoccupe),
  ...
  labs(x="Proportion of respondents", y="",
       title="Are you more or less concerned about climate change than five years ago?") +
  source_champ(nrow(drop_na(climat, pluspreoccupe)),
               "Interpretation: 44.6% of respondents are much more concerned about climate change than five years ago.")
```
...
## Personal position on ecology in general: catastrophe and collapse
...
data.frame(variable="Most environmental problems can be solved\n by applying more and better technology",
             ...
             data.frame(variable="Degrowth is necessary\n to face environmental challenges", valeur=climat$opinionecolo.decroissance),
             data.frame(variable="I agree that regulatory constraints (quotas, bans)\n should be put in place to protect the environment,\n even if it limits my comfort", valeur=climat$opinionecolo.contraintes),
             ...
             data.frame(variable="This type of disaster could cause\n a collapse of our societies*", valeur=climat$opinionecolo.effondrement2),
    ...
    mutate(variable=factor(variable, levels=unique(variable)),
           valeur=fct_relevel(valeur, c("Strongly agree", "Somewhat agree", "No opinion", "Somewhat disagree", "Strongly disagree")))
...
drop_na(dat) %>%
    ...
    source_champ(max(table(drop_na(dat)$variable)),
                  "* Those who responded negatively to the previous question are classified as « Strongly desagree ».
Lecture : 59% of respondents strongly agree with the statement\n that we will soon experience a major ecological catastrophe")
```
...
## Profound changes in the practice of our professions
```{r echo=FALSE}
pal <- RColorBrewer::brewer.pal(9, "RdYlBu")
...
## Reducing GHG emissions by one-third by 2030
```{r echo=FALSE}

ggplot(drop_na(climat, solreducrech),
       aes(y=fct_rev(solreducrech), x=..prop.., group=1)) +
    ...
    labs(x="Proportion des répondant·es", y="",
         title="France has committed to reducing its GHG emissions by one-third by 2030.\n In this respect, do you think that public research should:") +
  source_champ(nrow(drop_na(climat, solreducrech)),
...
climat$solreducrech3 <- fct_recode(climat$solreducrech,
  "More than a third" = "Set an example (reduce emissions by more than one-third)",
  "Around one-third" = "Reduce emissions by around one-third",
  "Less than one-third" = "Benefit from an exemption (reduce emissions by less than one-third)"
)
...
The analysis in this section are limited to PhD-level positions (including doctoral contracts)

```{r echo=FALSE}
# # sous base seulement recherche
climat_recherche <- subset(climat, ! sitpro %in% c(
  "Research support officer", "Research support assistant", "Technician",
...
colnames(table) <- c("Fieldwork, production and data collection",
                     "Research Stay",
                     "Workshop",
                     "Education, training, summer school",
                     "jury",
                     "financ",
                     "eval",
                     "autre",
...
table$Var2 <- fct_relevel(table$Var2,
                          c("Others","Education, training, summer school",
                            "Fieldwork, production and data collection",
                            "Workshop",
                            "Research Stay",
                            "Conference", 
                            "total"))
...
ggplot(table[table$Var2!="total",]) + 
  ...
  labs(x="", y="Distance traveled by plane in 2019 (km)", fill="")+
...
ggplot(table[table$Var2=="total",],
  ...
  # scale_fill_manual(values=rev(as.vector(cols[1:6])))+
  scale_fill_paletteer_d("ggthemes::excel_Celestial")+
  labs(x="", y="Distance traveled by plane (km)")
```


### Reasons for flying by statut
...
climat_recherche$sitpro_reduite <- fct_recode(climat_recherche$sitpro,
  "Doctoral contract" = "Doctorant·e CIFRE"
)
