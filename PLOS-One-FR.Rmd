---
title: 'Annexe 1 : tables et figures supplémentaires'
output:
  html_document:
    fig_width: 7.5
    fig_height: 4.5
...
# À ajouter dans le HTML pour éviter que les tableaux ne soient à cheval sur deux pages
<style type="text/css">
...
climat$solreducrech3 <- fct_recode(climat$solreducrech,
                                   "Plus d'un tiers" = "La recherche doit montrer l'exemple",
                                   "Un tiers environ" = "La recherche doit réduire ses émissions d'un tiers",
                                   "Moins d'un tiers" = "La recherche peut bénéficier d'un statut dérogatoire")
...
## Distribution de l'échantillon par discipline et statut
...
chisq.test(table(climat$preoccupe2 == "Extrêmement préoccupé·e", climat$NumVague)[, c(1, 5)])
```
...
chisq.test(table(climat$opinionecolo.cata == "Tout à fait d’accord", climat$NumVague)[, c(1, 5)])
...
chisq.test(table(climat$opinionecolo.contraintes == "Tout à fait d’accord", climat$NumVague)[, c(1, 5)])
```
...
chisq.test(table(climat$opinionecolo.proteger == "Tout à fait d’accord", climat$NumVague)[, c(1, 5)])
```
...
chisq.test(table(climat$solreducrech3 == "Réduire de plus d'un tiers", climat$NumVague)[, c(1, 5)])
```
...
chisq.test(table(climat$solinstit.limitevols == "C'est prioritaire", climat$NumVague)[, c(1, 5)])
```
...
chisq.test(table(climat$solinstit.selection == "C'est prioritaire", climat$NumVague)[, c(1, 5)])
```
...
chisq.test(table(climat$solinstit.conf == "C'est prioritaire", climat$NumVague)[, c(1, 5)])
```

- Financer le train même lorsque c’est plus cher ou allonge la durée de la mission
```{r echo=FALSE}
knitr::kable(round(cprop(table(climat$solinstit.train, climat$NumVague))))
chisq.test(table(climat$solinstit.train == "C'est prioritaire", climat$NumVague)[, c(1, 5)])
```
...
table_2 <- 
  tbl_regression(model_2, label=list(sexe ~ "Sexe", ageAgr ~ "Âge", sitpro_reduite2  ~ "Statut", discipline_agr4~"Discipline"),
                 intercept=TRUE) %>% 
  add_significance_stars() %>%
  bold_labels() %>%
  add_glance_table(include = c(nobs, r.squared, adj.r.squared))
...
table_3 <- 
  tbl_regression(model_3, label=list(sexe ~ "Sexe", ageAgr ~ "Âge", sitpro_reduite2  ~ "Statut", discipline_agr4~"Discipline", preoccupe2 ~ "Inquiétude concernant le chancement climatique"),
                 intercept=TRUE) %>% 
  add_significance_stars() %>%
  bold_labels() %>%
  add_glance_table(include = c(nobs, r.squared, adj.r.squared))
...
table_4 <- 
  tbl_regression(model_4, label=list(sexe ~ "Sexe", ageAgr ~ "Âge", sitpro_reduite2  ~ "Statut", discipline_agr4~"Discipline", solreducrech2 ~ "Réduction des émissions de GES dans la recherche d'ici 2030"),
                 intercept=TRUE) %>% 
  add_significance_stars() %>%
  bold_labels() %>%
  add_glance_table(include = c(nobs, r.squared, adj.r.squared))
...
table_5 <- 
  tbl_regression(model_5, label=list(sexe ~ "Sexe", ageAgr ~ "Âge", sitpro_reduite2  ~ "Statut", discipline_agr4~"Discipline", preoccupe2 ~ "Inquiétude concernant le chancement climatique", opinionecolo.cata ~ "Nous allons vivre une catastrophe écologique", opinionecolo.contraintes ~ "Contraintes réglementaires pour protéger l’environnement", opinionecolo.proteger ~ "Protéger l'environnement est plus important que protéger la croissance économique", solreducrech2 ~ "Réduction des émissions de GES dans la recherche d'ici 2030", solinstit.limitevols ~ "Imposer une limite au nombre de vols en avion par personne", solinstit.selection ~ "Ajouter les émissions de GES parmi les principaux critères de sélection des projets à financer", solinstit.conf ~ "Diminuer le poids des conférences et présentations à l’étranger dans les évaluations de carrière"),
                 intercept=TRUE) %>% 
  add_significance_stars() %>%
  bold_labels() %>%
  add_glance_table(include = c(nobs, r.squared, adj.r.squared))
...
table_6 <- 
  tbl_regression(model_6, label=list(sexe ~ "Sexe", ageAgr ~ "Âge", sitpro_reduite2  ~ "Statut", discipline_agr4~"Discipline", carriere ~ "Moment de la carrière où cherche à être promu, recruté ou titularisé", malpaye~ "Se sent mal payé", international.naiss ~ "Né à l'étranger"),
                 intercept=TRUE) %>% 
  add_significance_stars() %>%
  bold_labels() %>%
  add_glance_table(include = c(nobs, r.squared, adj.r.squared))
...
chisq.test(table(climat$discipline_agr4, climat$preoccupe2))
chisq.test(table(climat$discipline_agr4, climat$preoccupe2), simulate.p.value=TRUE, B=10000)
chisq.test(table(climat$sitpro_reduite, climat$preoccupe2))
chisq.test(table(climat$sitpro_reduite, climat$preoccupe2), simulate.p.value=TRUE, B=10000)
```
...
## Opinions sur l'écologie en général
```{r echo=FALSE}
...
chisq.test(table(climat$discipline_agr4, climat$solreducrech3))
chisq.test(table(climat$chgtpratique, climat$solreducrech3))
```
...
chisq.test(table(climat$discipline_agr4, climat$solreducrech3))
chisq.test(table(climat$sitpro_reduite, climat$solreducrech3))
chisq.test(table(climat$sitpro_reduite, climat$solreducrech3), simulate.p.value=TRUE, B=10000)
```
...
climat_recherche <- subset(climat, ! sitpro %in% c(
  ...
  "Autre"))
climat_recherche$sitpro_reduite <- droplevels(climat_recherche$sitpro_reduite)
```
...
ggplot(table[table$Var2!="Total",]) + 
  ...
  scale_fill_manual(values=paletteer_d("ggthemes::excel_Celestial")[c(1, 2, 4, 5, 3, 6)])+
  labs(x="", y="Distance parcourue en avion en 2019 (km)", fill="")+
  guides(fill=guide_legend(reverse = TRUE))
```

ANOVA et test post-hoc Tukey-Kramer:
```{r echo=FALSE}
lm(volsdist_tot ~ discipline_agr4, climat_recherche, contrasts=list(discipline_agr4="contr.Sum")) %>%
  tbl_regression(label=list(discipline_agr4 ~ "Discipline",
                            "(Intercept)" ~ "Constante (moyenne générale)"),
                 intercept=TRUE) %>% 
  bold_labels() %>%
  add_glance_table(include = c(nobs, p.value, r.squared, adj.r.squared))
knitr::kable(TukeyHSD(aov(volsdist_tot ~ discipline_agr4, climat_recherche))$discipline_agr4)
```
...
ANOVA et test post-hoc Tukey-Kramer :
```{r echo=FALSE}
lm(volsdist_tot ~ sitpro_reduite, climat_recherche, contrasts=list(sitpro_reduite="contr.Sum")) %>%
  tbl_regression(label=list(sitpro_reduite ~ "Discipline",
                            "(Intercept)" ~ "Intercept (moyenne générale)"),
                 intercept=TRUE) %>% 
  bold_labels() %>%
  add_glance_table(include = c(nobs, p.value, r.squared, adj.r.squared))
knitr::kable(TukeyHSD(aov(volsdist_tot ~ sitpro_reduite, climat_recherche))$sitpro_reduite)
```
...
chisq.test(table(climat$discipline_agr4, climat$ordis.5anstotalproplus0))
chisq.test(table(climat$discipline_agr4, climat$ordis.nbtotalproplus1))

chisq.test(table(climat$discipline_agr4, climat$ordis.tousindisp))
chisq.test(table(climat$sitpro_reduite, climat$ordis.5anstotalproplus0)[c("Directeur·rice de recherche", "ATER"),])
```
...
```{r}
chisq.test(table(climat$discipline_agr4, climat$solrisqreducavion.qual, exclude=c(NA, "Non concerné·e")))

chisq.test(table(climat$discipline_agr4, climat$solrisqreducavion.donnees, exclude=c(NA, "Non concerné·e")))
chisq.test(table(climat$discipline_agr4, climat$solrisqreducavion.donnees, exclude=c(NA, "Non concerné·e")), simulate.p.value=TRUE, B=10000)

chisq.test(table(climat$sitpro_reduite, climat$solrisqreducavion.diffusion, exclude=c(NA, "Non concerné·e")))
chisq.test(table(climat$sitpro_reduite, climat$solrisqreducavion.diffusion, exclude=c(NA, "Non concerné·e")), simulate.p.value=TRUE, B=10000)

chisq.test(table(climat$sitpro_reduite, climat$solrisqreducavion.isoler, exclude=c(NA, "Non concerné·e")))
chisq.test(table(climat$sitpro_reduite, climat$solrisqreducavion.isoler, exclude=c(NA, "Non concerné·e")), simulate.p.value=TRUE, B=10000)
