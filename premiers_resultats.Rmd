...
+             data.frame(variable="vous êtes encouragé·e à prendre le train plutôt que l'avion", valeur=climat$labopratiques.train),
+              data.frame(variable="il existe une charte/un groupe de travail/un·e responsable pour réduire l'empreinte écologique des activités de recherche", valeur=climat$labopratiques.charte),
+              data.frame(variable="des mesures ont été mises en oeuvre pour limiter l'impact environnemental des pots/buffets", valeur=climat$labomesures.pots)) %>%
+     mutate(variable=factor(variable, levels=unique(variable)),
+           valeur=fct_relevel(valeur, c("oui", "non, mais la question a été abordée", "non concerné·e", "non", "je ne sais pas")))
Problem with `mutate()` input `valeur`.
ℹ Unknown levels in `f`: oui, non, mais la question a été abordée, non concerné·e, non, je ne sais pas
ℹ Input `valeur` is `fct_relevel(...)`.Unknown levels in `f`: oui, non, mais la question a été abordée, non concerné·e, non, je ne sais pas> drop_na(dat) %>%
+     ggplot(aes(x=variable)) +
+    geom_bar(aes(fill=fct_rev(valeur)), position="fill") +
+    scale_fill_manual(values=c("#d7191c", "light grey", "#a6d96a", "#1a9641", "light grey")) +
...
+          title="Actions mises en oeuvre dans les laboratoires/unités de recherche :",
+         caption="Source : Enquête « Les personnels de la recherche face au changement climatique », Labos 1point5, 2020\nChamp : personnels affiliés à une unité du CNRS "

#reliquat
#ceux là ils marchaient ce matin, après ils ont disparu et je les ai récupérés dans la console mais ils marchent plus et je ne sais pas pourquoi
```{r echo=FALSE}
      pal <- RColorBrewer::brewer.pal(9, "RdYlBu")
