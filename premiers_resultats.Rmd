...
data.frame(statut=factor(names(repstatut)), taux=repstatut) %>%
  ...
  labs(x="Proportion", y="",
       ...
       caption="Lecture : 34% des ingénieur.es de recherche à qui une invication a été envoyée
...
data.frame(statut=factor(names(repinstitut)), taux=repinstitut) %>%
  ...
  labs(x="Proportion des invitations envoyées", y="",
       ...
       caption="Lecture : 31% des personnes affiliées à l'INSU à qui une invication a été envoyée
       ont répondu au moins à la première page du questionnaire.")
```
...
ggplot(drop_na(climat, statut),
  ...
  labs(x="Proportion des répondant·es", y="",
       ...
       caption="Lecture : 65,8% des répondant·es sont fonctionnaires.") +
...
ggplot(drop_na(climat, sitpro),
  ...
  labs(x="Proportion des répondant·es", y="",
       ...
       caption="Lecture : 13% des répondant·es sont doctorant·es contractuel·les.") +
...
ggplot(drop_na(climat, discipline_agr),
  ...
  labs(x="Proportion des répondant·es", y="",
       ...
       caption="Lecture : 15% des répondant·es ont déclaré comme discipline principale une discipline relevant des sciences humaines.") +
...
ggplot(drop_na(climat, acthum3),
  ...
  labs(x="Proportion des répondant·es", y="",
       ...
       caption="Lecture : 95,1% des répondant·es ont déclaré que les activités humaines sont
       l'unique cause ou, du moins, jouent un grand rôle dans le changement climatique.") +
...
ggplot(drop_na(climat, preoccupe),
  ...
  labs(x="Proportion des répondant·es", y="",
       ...
       caption="Lecture : 31,7% des répondant·es se déclarent extrêmement préocupé·es par le changement climatique.") +
...
ggplot(drop_na(climat, pluspreoccupe),
  ...
  labs(x="Proportion des répondant·es", y="",
       ...
       caption="Lecture : 44,6% des répondant·es se déclarent beaucoup plus préocupé·es par le changement climatique qu'il y a cinq ans.") +
...
ggplot(drop_na(climat, chgtpratique),
  ...
  labs(x="Proportion des répondant·es", y="",
       ...
       caption="Lecture: 47% des répondant·es se déclarent 
tout à fait d'accord avec l'idée selon laquelle l'urgence climatique exige 
des changements profonds dans la pratique de nos métiers.
...
drop_na(dat) %>%
    ...
    scale_fill_manual(values=c("#d7191c", "grey70", "grey40", "#a6d96a", "#1a9641")) +
     ...
     labs(y="Proportion des répondant·es", x="",
          ...
          dans les laboratoires/unités de recherche :", caption="Environ 80% des répondant·es déclarent pouvoir trier leurs déchets dans leur laboratoire.") +
     source_champ()
```
...
ggplot(infoTot,  aes(x=prop, y=X1 )) +
  ...
  labs(y="", x="Part de l'ensemble des ordinateurs et tablettes",
       title="", caption="75% des ordinateurs et des tablettes sont jugées 
       indispensables par leur utilisateu·rice.") +
  source_champ()
...
ggplot(informatique,  aes(x=prop, y=fct_reorder(TypeOrdi, prop), fill= set )) +
  ...
  labs(y="", x="Part de l'ensemble des ordinateurs et tablettes", fill="",
       ...
       caption="Lecture: Sur 100 tablettes et ordinateurs (utilisés professionnellement par les répondant.es ?), 14,3 sont des ordinateurs partagés au laboratoire.
Sur 100 tablettes et ordinateurs, 
11,6 sont des ordinateurs partagés au laboratoire qui ont moins de 5 ans. 5,8 sont des ordinateurs partagés au laboratoire jugés indispensables.
Source : Enquête « Les personnels de la recherche face au changement climatique »,\nLabos 1point5, 2020\nChamp : personnels affiliés à une unité du CNRS")
...
ggplot(chgtOrdi,  aes(x=prop, y=fct_reorder(TypeRaisonChgtOrdi, prop) )) +
  ...
  labs(y="", x="Proportion des répondant·es",
       ...
       leur ordinateur",
       caption="30% des répondant·es déclarent avoir changé leur dernier ordinateur car il ne fonctionnait plus très bien.") +
...
ggplot(drop_na(climat, reliquat.ordi),
 ...
 alors que ce n'était pas vraiment indispensable  ?", caption="20% des répondant·es déclarent avoir déjà utilisé, dans les cinq dernières années, un reliquat de budget pour acheter un ordinateur (ou une tablette) alors que cet achat ne leur paraissait pas vraiment indispensable.") + 
...
ggplot(drop_na(climat, solevolges.conf),
  ...
  geom_text(aes(label=scales::percent(..prop.., accuracy=1, decimal.mark=",")),
            ...
            hjust=-0.2, size=3) +
  labs(x="Proportion des répondant·es", y="",
       title="Au cours des 5 dernières années (hors confinement), diriez vous que vos émissions\nde gaz à effet de serre liées à vos vols en avion pour les conférences, réunions ont...", caption= "Lecture: 6% des répondant·es déclarent qu'au cours des cinq dernières années,
       leurs émissions liées à des vols en avion ont fortement augmenté.") +
...
ggplot(drop_na(climat, volshTot),
  ...
  geom_text(aes(label=scales::percent(..prop.., accuracy=1, decimal.mark=",")),
            ...
            hjust=-0.2, size=3) +
  labs(x="Proportion des répondant·es", y="",
       title="Heures de vol en avion dans le cadre professionnel en 2019", caption= "Lecture: 75% des répondant·es déclarent n'avoir pas volé au cours des cinq dernières années (0h de vol), 10% ont volé mais moins de 10h.") +
...
ggplot(drop_na(climat, vols2ans),
  ...
  labs(x="Proportion parmi les répondant·es n'ayant pas pris l'avion en 2019",
       ...
       caption= "Lecture: 66% des répondant·es n'ayant pas pris l'avion en 2019 ne l'ont pas non plus pris en 2017 et 2018, et 15% l'ont pris pour une durée total de moins de 10h sur ces deux années.",
...
ggplot(DFNbJourTravailles,  aes(x=prop, y=volsjours )) +
  ...
  labs(y="", x="Part des déplacements en avion",
       ...
       selon le temps travaillé sur place", caption= "Lecture: Sur 100 déplacements professionnels en avion, 5 l'ont été pour faire un séjour de plus d'un mois, 59 pour un séjour de deux jours à une semaine.") +
  source_champ()
...
ggplot(DFNbJourTravailles,  aes(x=prop, y=volsjours )) +
  ...
  labs(y="", x="Part du total des distances parcourues en avion",
       ...
       caption= "Lecture: 12% des distances parcourues en avion l'ont été pour faire un séjour de plus d'un mois. Autrement dit, sur 100 km parcourus en avion, 12 le sont pour faire des séjours de plus d'un mois.") +
  source_champ()
...
ggplot(DFDistAvion,  aes(x=prop, y=fct_reorder(volsmotif, prop) )) +
  ...
  labs(y="",
       ...
       title="Motif principal pour les déplacements professionnels\neffectués en avion", caption= "Lecture: Sur 100 déplacements professionnels en avion, 40 l'ont été pour faire une présentation dans une conférence.") +
  source_champ()
...
ggplot(DFDistAvion,  aes(x=prop, y=fct_reorder(volsmotif, prop) )) +
  ...
  labs(y="", x="Part du total des distances parcourues en avion",
       ...
       caption= "Lecture: 43% des distances parcourues en avion l'ont été pour faire des présentations dans des conférences. Autrement dit, sur 100 km parcourus en avion, 43 le sont pour ce motif.") +
  source_champ()
  ...
  geom_col() +
    labs(y="",
       ...
       x="Distance moyenne parcourue en avion (en km)",
        caption="En moyenne, les déplacements en avion liés au terrain ou au recueil 
de données font un peu plus de 10000 km, 
contre environ 7000 pour les déplacements liés aux conférences.") +
  source_champ()
...
ggplot(DFDistAvion,  aes(x=prop, y=fct_reorder(volstrain, prop) )) +
  ...
  labs(y="", x=bquote("Part du total des émissions en" ~CO[2] *"e"),
       ...
       caption="0,2% des émissions de gaz à effet de serre liées à des déplacements en avion correspondent à des déplacements qui auraient pu être réalisés en moins de 3h en train.") +
  source_champ()
...
ggplot(raisons, aes(x=prop, y=fct_reorder(TypeRaison, prop))) +
  ...
  labs(y="", x="Proportion parmi les répondant·es concerné·es",
       ...
       caption="Pour 66% des répondant·es, le fait que l'avion soit plus rapide fait partie des raisons pour lesquelles ils ou elles ont pris l'avion pour un trajet qui prend moins de 6h en train.") +
  source_champ(sum(aumoinsunecochee))
```
...
ggplot(drop_na(climat, renoncedep.env),
  ...
  labs(x="Proportion des répondant·es", y="", 
       ...
       professionnels à l’étranger pour des considérations
       environnementales ?" )+
...
Source : Enquête « Les personnels de la recherche face au changement climatique », Labos 1point5, 2020
Champ : personnels permanents affiliés à une unité du CNRS, utilisant un dispositif expérimental")
```
...
drop_na(dat) %>%
    ...
    labs(y="Proportion parmi les répondant·es utilisant un dispositif expérimental", x="",
         ...
         caption="Note : la proportion de personnes concernées varie selon la question
Source : Enquête « Les personnels de la recherche face au changement climatique », Labos 1point5, 2020
Champ : personnels affiliés à une unité du CNRS, utilisant un dispositif expérimental")
```
