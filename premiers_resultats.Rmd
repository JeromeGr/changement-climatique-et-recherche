...
author:
- name: Équipe « Représentations et pratiques » de Labos 1point5
- name: Marianne Blanchard (Université Toulouse Jean-Jaurès, CERTOP)
- name: Milan Bouchet-Valat (Ined)
- name: Damien Cartron (CNRS, CMH)
- name: Jérôme Greffion (Université Paris Nanterre, IDHES)
- name: Julien Gros (CNRS, LEST)
---


![](Logo Labos 1.5.svg){width=200px}
...
Ce document est complémentaire de la présentation générale de l'enquête disponible **ici (LIEN À INSÉRER)**. Il vise à rendre rapidement publiques les réponses à une sélection de questions, sans avoir recours à des traitements statistiques avancés et sans fournir d'interprétation. Il s'agit d'un document d'étape qui pourra être modifié et complété par la suite.
...
ggplot(infoTot,  aes(x=prop, y=X1 )) +
  ...
  source_champ(sum(rowSums(!is.na(select(climat, starts_with("ordis.")))) > 0),
               "75% des ordinateurs et des tablettes sont jugé·es indispensables par leur utilisateur·ice")
...
informatique2$set[informatique2$set=="5ans"]<-"Dont de moins de 5 ans"
informatique2$set[informatique2$set=="indisp"]<-"Dont jugé·es indispensables"
...
11,6 sont des ordinateurs partagés jugés indispensables, et 5,8 sont des ordinateurs partagés qui ont moins de 5 ans")
...
leur ordinateur") +
  source_champ(sum(aumoinsunecochee),
               "30% des répondant·es ont changé leur dernier ordinateur car il ne fonctionnait plus très bien")
...
```{r echo=FALSE}
ggplot(drop_na(climat, volsh),
       aes(y=fct_rev(volsh), x=..prop.., group=1)) +
  geom_bar() +
  scale_x_continuous(labels=function(x) scales::percent(x, 1)) +
  coord_cartesian(xlim=c(0, 0.78)) +
  geom_text(aes(label=scales::percent(..prop.., accuracy=1, decimal.mark=",")),
  ...
  source_champ(nrow(drop_na(climat, volsh)),
               "Lecture : 50% des répondant·es n'ont pas volé au cours des cinq dernières années, et 25% ont volé mais moins de 10h")
```
...
ggplot(DFNbJourTravailles,  aes(x=prop, y=volsjours )) +
  ...
  labs(y="", x="Part des déplacements en avion",
       title="Temps travaillé sur place lors d'un déplacement professionnel en avion en 2019") +
  source_champ(sum(climat$volsremplis, na.rm=TRUE),
               "Lecture : sur 100 vols professionnels en 2019, 5 correspondaient à un séjour de plus d'un mois")
...
ggplot(DFNbJourTravailles,  aes(x=prop, y=volsjours )) +
  ...
  labs(y="", x="Part du total des distances parcourues en avion",
       title="Part du total des distances parcourues en avion en 2019,
selon le temps travaillé sur place") +
  source_champ(sum(climat$volsremplis, na.rm=TRUE),
               "Lecture : sur 100 km parcourus en avion en 2019, 12 km l'ont été pour des séjours de plus d'un mois")
```
...
ggplot(DFDistAvion,  aes(x=prop, y=fct_reorder(volsmotif, prop) )) +
  ...
  labs(y="",
       ...
       title="Motif principal des déplacements professionnels en avion en 2019") +
  source_champ(sum(climat$volsremplis, na.rm=TRUE),
...
ggplot(DFDistMoyAvion,  aes(x=DistMoy, y=fct_reorder(volsmotif, DistMoy) )) +
  geom_col() +
    labs(y="",
         title="Distance moyenne d'un trajet aller-retour en avion en 2019\nselon le motif principal du déplacement professionnel",
...
Les temps de trajet en train sont obtenus à partir des horaires de la SNCF (ou des entreprises équivalentes pour l'étranger). Les émissions de gaz à effet de serre sont calculées à partir de la distance orthodromique + 95 km, selon la méthode [GES 1point5](https://labos1point5.org/ges-1point5). Note : les vols intérieurs avec une correspondance peuvent ne pas avoir été déclarés séparément par les répondant·es, ce qui peut entraîner une sous-estimation des émissions des vols courts.
...
ggplot(drop_na(climat, trainavion),
  ...
  source_champ(nrow(drop_na(climat, trainavion)),
               "Lecture : dans les cinq dernières années, 36% des répondant·es ont pris le train plutôt que l'avion,\nalors que le trajet était plus long, pour des raisons environnementales")
```
...
professionnels à l’étranger pour des considérations environnementales ?") +
  source_champ(nrow(drop_na(climat, renoncedep.env)),
               "Lecture : en 2019, 14% des répondant·es ont renoncé à un déplacement professionnel\nprincipalement pour des considérations environnementales")
```
...
jours travaillés le week-end") +
  source_champ(nrow(drop_na(climat, joursdomicile)),
               "Lecture : 48% des répondant·es ne travaillent jamais de leur domicile")
```
...
par l'ensemble des répondant·es, par mode de transport") +
  source_champ(sum(climat$tpsdomtravrempli),
               "Lecture : sur 100 heures de trajet domicile-travail, les répondant·es en passent 24 seul·es dans leur voiture")
```
...
ggplot(Covoit,  aes(x=prop, y=fct_reorder(RaisonsPasCovoit, prop) )) +
  ...
  labs(y="", x="Proportion parmi les répondant·es autosolistes",
       title="Raison(s) de ne pas faire du covoiturage\npour les trajets domicile-travail",
       caption=sprintf("Lecture : 44%% des répondant·es qui prennent la voiture seul·es le font parce qu'elles/ils tiennent à la flexibilité de leurs horaires
Source : Enquête « Les personnels de la recherche face au changement climatique », Labos 1point5, 2020\nChamp : personnels affiliés à une unité du CNRS prenant la voiture seuls pour leurs trajets domicile-travail (n=%i)", sum(aumoinsunecochee)))
...
drop_na(dat) %>%
  ...
  geom_text(aes(label=scales::percent(valeur, accuracy=1, decimal.mark=",")),
            ...
            hjust=-0.2, size=3) +
  labs(y="", x="Proportion des répondant·es",
       title="Part des répondant·es n'ayant jamais testé chaque usage de la visioconférence") +
  source_champ(max(table(drop_na(dat)$variable)),
               "Lecture : 26% des répondant·es n'ont jamais testé la visioconférence pour un oral de jury")
...
vous semblent-elles adaptées ?",
       caption=sprintf("Lecture : 54%% des répondant·es ayant déjà testé la visioconférence pour une réunion de 3 à 5 personnes\nla trouvent très adaptée à cet usage
...
dat <- rbind(data.frame(variable="Pouvoir faire autre chose\nen même temps que la réunion",
             ...
             data.frame(variable="Gagner du temps",
...
privilégier la visioconférence ou l’audioconférence à un déplacement ?") +
  source_champ(max(table(drop_na(dat)$variable)),
               "Lecture : pour 62% des répondant·es, pouvoir réunir de nombreuses personnes éloignées\nest une raison pouvant beaucoup les pousser à privilégier la visioconférence")

```

## Raisons de ne pas privilégier la visioconférence 

```{r echo=FALSE, fig.height=5}

dat <- rbind(data.frame(variable="C’est plus fatigant à suivre qu’une réunion 
                        en présentiel", 
                        valeur=climat$visiopb.fatigue),
             data.frame(variable="Cela génère des problèmes techniques",
                        valeur=climat$visiopb.technique),
             data.frame(variable="Cela rend difficile d’écrire ou dessiner 
                        en discutant (schémas, formules, équations...)", 
                        valeur=climat$visiopb.ecrire),
             data.frame(variable="Cela limite les aspects relationnels", 
                        valeur=climat$visiopb.relation))%>%
  mutate(variable=factor(variable, levels=unique(variable)),
         valeur=fct_relevel(valeur, c(
           "Oui, beaucoup", 
           "Oui, un peu", 
...
drop_na(dat) %>%
  ggplot(aes(x=variable)) +
  geom_bar(aes(fill=fct_rev(valeur)), position="fill") +
  scale_fill_manual(values=c("#1a9641", "#a6d96a", "light grey", "#fdae61", "#d7191c"))+
  scale_y_continuous(labels=scales::percent_format(),
                     sec.axis=sec_axis(~ 1-., labels=scales::percent_format())) +
  coord_flip() +
  theme(legend.position="bottom") +
  guides(fill=guide_legend(reverse = TRUE, nrow=2, byrow=TRUE)) +
  labs(y="Proportion des répondant·es", x="", fill="",
       title="Imaginons une réunion d’une journée, située à 2h de votre
domicile en voiture. Les raisons suivantes pourraient-elles
vous empêcher de privilégier la visioconférence
ou l’audioconférence à un déplacement ?") +
  source_champ(max(table(drop_na(dat)$variable)),
               "Lecture : pour 17% des répondant·es, les problèmes techniques sont une raison\npouvant beaucoup les empêcher de privilégier la visioconférence")
...
ou de l’audioconférence dans le cadre professionnel ?") +
  source_champ(nrow(drop_na(climat, visioavtconf)),
               "Lecture : 30,5% des répondant·es n'avaient jamais usage de la visioconférence avant le confinement")
```

## Usage pendant le confinement de la visioconférence

```{r echo=FALSE}

ggplot(drop_na(climat, visiopdtconf),
       aes(y=fct_rev(visiopdtconf), x=..prop.., group=1)) +
  geom_bar() +
  scale_x_continuous(labels=function(x) scales::percent(x, 1)) +
  coord_cartesian(xlim=c(0, 0.52)) +
  geom_text(aes(label=scales::percent(..prop.., accuracy=0.1, decimal.mark=",")),
            stat="count", position=position_dodge(.9),
            hjust=-0.2, size=3) +
  labs(x="Proportion des répondant·es", y="", title="Pendant le confinement quel a été votre usage de la visioconférence 
ou de l’audioconférence dans le cadre professionnel ?") +
  source_champ(nrow(drop_na(climat, visiopdtconf)),
               "Lecture : 2,8% des répondant·es n'ont jamais utilisé la visioconférence pendant le confinement")
```
...
ou moins favorable à la visioconférence ou l’audioconférence ?") +
  source_champ(nrow(drop_na(climat, visioapresconf)),
               "Lecture : 29% des répondant·es sont beaucoup plus favorables à la visioconférence qu'avant le confinement")
...
ggplot(drop_na(climat, solcovoit),
    ...
    labs(x="Proportion parmi les répondant·es prenant la voiture", y="",
         ...
         caption=sprintf("Lecture : 24%% des répondant·es qui prennent la voiture pour leurs trajets domicile-travail\nrecourraient certainement au covoiturage s'il était organisé à l'échelle du laboratoire
Source : Enquête « Les personnels de la recherche face au changement climatique », Labos 1point5, 2020
...
drop_na(dat) %>%
    ...
    source_champ(max(table(drop_na(dat)$variable)),
                 "Lecture : 64% des répondant·es ont choisi la bonne estimation des émissions liées à 3h de visioconférence, ou une estimation proche")
```
...
drop_na(dat) %>%
    ...
    labs(y="Proportion des répondant·es", x="",
         title="Estimations des émissions de gaz à effet de serre") +
    source_champ(max(table(drop_na(dat)$variable)),
                 "Lecture : 26% des répondant·es ont choisi la bonne estimation des émissions liées à 3h de visioconférence")
```