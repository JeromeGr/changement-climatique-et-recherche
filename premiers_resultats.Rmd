...
ggplot(DFDistAvion,  aes(x=prop, y=volsmotif )) +
  geom_col() +
  scale_x_continuous(labels=function(x) scales::percent(x, 1)) +
  labs(y="",
       x="Proportion des déplacements effectués en avion par les répondant·es",
...
ggplot(DFDistAvion,  aes(x=prop, y=fct_reorder(volsmotif, prop) )) +
  geom_col() +
  scale_x_continuous(labels=function(x) scales::percent(x, 1)) +
  labs(y="", x="Part de la distance parcourue en avion par les répondant·es",
...
Proportion de la distance totale parcourue en avion par les répondant·es avec des trajets en train de moins de 3h (estimation)
...
ggplot(TpsTransportDomT,  aes(x=prop, y=fct_reorder(TypeTransport, prop) )) +
  ...
  scale_x_continuous(labels=scales::percent_format()) +
  labs(y="",
       x="Proportion des répondant·es ayant pris l'avion en 2019",
...
```{r echo=FALSE}
dat <- rbind(data.frame(variable="Parcourir 50 km en voiture pour aller et revenir du travail\npendant un an (soit environ 12 000 km)", valeur=climat$quizfacteurs.voiture2),
             data.frame(variable="Fabriquer un ordinateur portable neuf", valeur=climat$quizfacteurs.ordi2),
             data.frame(variable="Imprimer une thèse de 200 pages en 10 exemplaires,\nrecto-verso", valeur=climat$quizfacteurs.these2),
             data.frame(variable="Faire un aller-retour Paris – New York en avion\n(soit environ 12 000 km)", valeur=climat$quizfacteurs.avion2),
             data.frame(variable="Produire un steak de bœuf de 150 g", valeur=climat$quizfacteurs.steak2),
             data.frame(variable="Faire un aller-retour Paris-Marseille en TGV\n(soit 1600 km)", valeur=climat$quizfacteurs.TGV2),
             data.frame(variable="Faire 3 heures de visioconférence avec votre ordinateur\n(pour une personne)", valeur=climat$quizfacteurs.visio2)) %>%
    mutate(variable=factor(variable, levels=unique(variable)))
...
dat <- rbind(data.frame(variable="Parcourir 50 km en voiture pour aller et revenir du travail\npendant un an (soit environ 12 000 km)", valeur=climat$quizfacteurs.voiture3),
             data.frame(variable="Fabriquer un ordinateur portable neuf", valeur=climat$quizfacteurs.ordi3),
             data.frame(variable="Imprimer une thèse de 200 pages en 10 exemplaires,\nrecto-verso", valeur=climat$quizfacteurs.these3),
             data.frame(variable="Faire un aller-retour Paris – New York en avion\n(soit environ 12 000 km)", valeur=climat$quizfacteurs.avion3),
             data.frame(variable="Produire un steak de bœuf de 150 g", valeur=climat$quizfacteurs.steak3),
             data.frame(variable="Faire un aller-retour Paris-Marseille en TGV\n(soit 1600 km)", valeur=climat$quizfacteurs.TGV3),
             data.frame(variable="Faire 3 heures de visioconférence avec votre ordinateur\n(pour une personne)", valeur=climat$quizfacteurs.visio3)) %>%
    mutate(variable=factor(variable, levels=unique(variable)))
