...
table_4 <- 
  tbl_regression(model_4, label=list(sexe ~ "Sex", ageAgr ~ "Age", sitpro2  ~ "Status", discipline_agr4~"Discipline", solreducrech2 ~ "Desirable GHG emission reduction target for public research by 2030")) %>% 
  add_significance_stars() %>%
  add_glance_table(include = nobs)
...
res.reg5 <- lm(vaguenum ~ sexe + ageAgr + sitpro2  + discipline_agr4 + preoccupe2 + opinionecolo.cata+ opinionecolo.contraintes +       opinionecolo.proteger + solreducrech2 + solinstit.limitevols + solinstit.selection + solinstit.conf, data=climatRegr )
res.reg6 <- lm(vaguenum ~ sexe + ageAgr + sitpro2  + discipline_agr4 + carriere + malpaye + international.naiss, data=climatRegr )
#res.reg7 <- lm(vaguenum ~ sexe + ageAgr + sitpro2  + discipline_agr4 + nbpublistranch2 + tpsquotiteNum + carriere + malpaye + international.naiss, data=climatRegr )


htmlreg(list(res.reg1, res.reg2, res.reg3, res.reg4, res.reg5,res.reg6), 
        stars = c(0.001, 0.01, 0.05, 0.1), digits = 2, #single.row = TRUE,
        custom.coef.map = list("sexeFemme"= "Woman (ref = man)",
                               "sexeAutre"="Sex : other",
                               ...
                               "solreducrech2"="Recherche doit réduire émissions de GES de 1/3 environ (Ref = Recherche doit montrer l'exemple, réduire de plus d'1/3)",
                               "solreducrech2Recherche peut bénéficier d'un statut dérogatoire"="Recherche peut bénéficier d'un statut dérogatoire",
                              
                               "preoccupe2Extrêmement préoccupé·e"="Extrêmement préoccupé·e par le changement climatique (Ref = Très préoccupé·e)",
                               "preoccupe2Assez préoccupé·e"="Assez préoccupé·e par le changement climatique",
                               "preoccupe2Un peu préoccupé·e"="Un peu préoccupé·e par le changement climatique",
                               "preoccupe2Pas du tout préoccupé·e"="Pas du tout préoccupé·e par le changement climatique",
                               "preoccupe2Sans opinion"="Sans opinion quant à l'inquiétude sur le changement climatique",
                               
                               "opinionecolo.cataPlutôt d'accord"="Plutôt d'accord qu'on va bientôt vivre une catastrophe écologique majeure (Ref = Tout à fait d'accord)",
                               "opinionecolo.cataPlutôt pas d'accord"="Plutôt pas d'accord qu'on va bientôt vivre une catastrophe écologique majeure",
                               "opinionecolo.cataPas du tout d'accord"="Pas du tout d'accord qu'on va bientôt vivre une catastrophe écologique majeure",
                               "opinionecolo.cataSans opinion"="Sans opinion sur la catastrophe écologique",
                               
                               "opinionecolo.contraintesPlutôt d'accord"="Plutôt d'accord que des contraintes réglementaires soient mises en place pour l'environnement même si ça limite le confort (Ref = Tout à fait d'accord)",
                               "opinionecolo.contraintesPlutôt pas d'accord"="Plutôt pas d'accord que des contraintes réglementaires soient mises en place pour l'environnement même si ça limite le confort",
                               "opinionecolo.contraintesPas du tout d'accord"="Pas du tout d'accord que des contraintes réglementaires soient mises en place pour l'environnement même si ça limite le confort",
                               "opinionecolo.contraintesSans opinion"="Sans opinion sur les contraintes réglementaires",
                               
                               "opinionecolo.protegerPlutôt d'accord"="Plutôt d'accord qu'il est plus important de protéger l'environnement plutôt que la croissance (Ref = Tout à fait d'accord)",
                               "opinionecolo.protegerPlutôt pas d'accord"="Plutôt pas d'accord qu'il est plus important de protéger l'environnement plutôt que la croissance",
                               "opinionecolo.protegerPas du tout d'accord"="Pas du tout d'accord qu'il est plus important de protéger l'environnement plutôt que la croissance",
                               "opinionecolo.protegerSans opinion"="Sans opinion sur l'arbitrage environnement/croissance",
                               
                               "solinstit.limitevolsC'est secondaire"="C'est secondaire d'imposer une limite de vols en avion par personne (Ref =C'est prioritaire)",
                               "solinstit.limitevolsIl ne faut pas le mettre en œuvre"="Il ne faut pas imposer une limite de vols en avion par personne",
                               "solinstit.limitevolsSans opinion"="Sans opinion sur la limite de vols en avion par personne",
                               
                               "solinstit.selectionC'est secondaire"="C'est secondaire d'ajouter les GES parmis les critères de sélection des projets à financer (Ref =C'est prioritaire)",
                               "solinstit.selectionIl ne faut pas le mettre en œuvre"="Il ne faut pas ajouter les GES parmis les critères de sélection des projets à financer",
                               "solinstit.selectionSans opinion"="Sans opinion sur la prise en compte des émissions de GES pour sélection les projets à financer",
        
                               "solinstit.confC'est secondaire"="C'est secondaire de diminuer le poids des conférences à l'étranger dans l'évaluation des carrières (Ref =C'est prioritaire)",
                               "solinstit.confIl ne faut pas le mettre en œuvre"="Il ne faut pas diminuer le poids des conférences à l'étranger dans l'évaluation des carrières ",
                               "solinstit.confSans opinion"="Sans opinion sur la diminution du poids des conférences à l'étranger dans l'évaluation des carrières",
        
                               "nbpublistranch2(0.5,2]"="Une à deux publications depuis 2017 (Ref = Aucune publication )",
                               "nbpublistranch2(2,4]"="2 à 4 publications",
                               "nbpublistranch2(4,7]"="5 à 7 publications",
                               "nbpublistranch2(7,12]"="8 à 12 publications",
                               "nbpublistranch2(12,20]"="13 à 20 publications",
                               "nbpublistranch2(20,40]"="21 à 40 publications",
                               "nbpublistranch2(40,180]"="40 à 180 publications",
                               
                               "tpsquotiteNum"="Quotité de temps partiel",
                               "carriereOui"="Etre à un moment de la carrière où on cherche à être promu, recruté, titularisé",
                               "international.naissNon"="Né.e à l'étranger",
                               "malpayeOui"="S'estime mal payé"),
        symbol = "+",
        caption = "Tableau 2 : Régressions linéaires multiples sur le nombre de messages envoyés aux répondants avant qu'ils répondent", caption.above=TRUE, 
        single.row = TRUE)
```
